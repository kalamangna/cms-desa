...
return new class extends Migration
{
        ...
        try {
            ...
            Family::withTrashed()->chunk(100, function ($families) {
                foreach ($families as $family) {
                    ...
                    if ($bt !== $family->building_type) {
                        $updates['building_type'] = $bt;
                    }

                    $op = $this->parseOwnershipProof($family->ownership_proof);
                    if ($op !== $family->ownership_proof) {
                        $updates['ownership_proof'] = $op;
                    }

                    $fm = $this->parseFloorMaterial($family->floor_material);
                    if ($fm !== $family->floor_material) {
                        $updates['floor_material'] = $fm;
                    }

                    $wm = $this->parseWallMaterial($family->wall_material);
                    if ($wm !== $family->wall_material) {
                        $updates['wall_material'] = $wm;
                    }

                    $rm = $this->parseRoofMaterial($family->roof_material);
                    if ($rm !== $family->roof_material) {
                        $updates['roof_material'] = $rm;
                    }

                    $fc = $this->parseCondition($family->floor_condition);
                    if ($fc !== $family->floor_condition) {
                        $updates['floor_condition'] = $fc;
                    }

                    $wc = $this->parseCondition($family->wall_condition);
                    if ($wc !== $family->wall_condition) {
                        $updates['wall_condition'] = $wc;
                    }

                    $rc = $this->parseCondition($family->roof_condition);
                    if ($rc !== $family->roof_condition) {
                        $updates['roof_condition'] = $rc;
                    }

                    $tf = $this->parseToiletFacility($family->toilet_facility);
                    if ($tf !== $family->toilet_facility) {
                        $updates['toilet_facility'] = $tf;
                    }

                    $ct = $this->parseClosetType($family->closet_type);
                    if ($ct !== $family->closet_type) {
                        $updates['closet_type'] = $ct;
                    }

                    $fd = $this->parseFecesDisposal($family->feces_disposal);
                    if ($fd !== $family->feces_disposal) {
                        $updates['feces_disposal'] = $fd;
                    }

                    $ws = $this->parseWaterSource($family->water_source);
                    if ($ws !== $family->water_source) {
                        $updates['water_source'] = $ws;
                    }

                    $ls = $this->parseLightingSource($family->lighting_source);
                    if ($ls !== $family->lighting_source) {
                        $updates['lighting_source'] = $ls;
                    }

                    $p1 = $this->parseElectricityPower($family->electricity_power_meter_1);
                    if ($p1 !== $family->electricity_power_meter_1) {
                        $updates['electricity_power_meter_1'] = $p1;
                    }

                    $p2 = $this->parseElectricityPower($family->electricity_power_meter_2);
                    if ($p2 !== $family->electricity_power_meter_2) {
                        $updates['electricity_power_meter_2'] = $p2;
                    }

                    $p3 = $this->parseElectricityPower($family->electricity_power_meter_3);
                    if ($p3 !== $family->electricity_power_meter_3) {
                        $updates['electricity_power_meter_3'] = $p3;
                    }
            ...
            Citizen::withTrashed()->chunk(200, function ($citizens) {
                foreach ($citizens as $citizen) {
                    ...
                    if ($dt !== $citizen->domicile_address_type) {
                        $updates['domicile_address_type'] = $dt;
                    }

                    $dw = $this->parseHasDigitalWallet($citizen->has_digital_wallet);
                    if ($dw !== $citizen->has_digital_wallet) {
                        $updates['has_digital_wallet'] = $dw;
                    }

                    $cs = $this->parseCitizenshipStatus($citizen->citizenship_status);
                    if ($cs !== $citizen->citizenship_status) {
                        $updates['citizenship_status'] = $cs;
                    }

                    $bp = $this->parseBpjsStatus($citizen->bpjs_status);
                    if ($bp !== $citizen->bpjs_status) {
                        $updates['bpjs_status'] = $bp;
                    }

                    $fr = $this->parseFamilyRelation($citizen->family_relation);
                    if ($fr !== $citizen->family_relation) {
                        $updates['family_relation'] = $fr;
                    }
    ...
    private function parseBuildingType(?string $val): ?string
    {
        ...
        if (str_contains($clean, 'tunggal')) {
            return 'Rumah Tinggal Tunggal';
        }
        if (str_contains($clean, 'deret')) {
            return 'Rumah Tinggal Tunggal';
        }
    ...
    private function parseFloorMaterial(?string $val): ?string
    {
        ...
        if (str_contains($clean, 'keramik') || str_contains($clean, 'kramik')) {
            return 'Keramik';
        }
        if (str_contains($clean, 'ubin') || str_contains($clean, 'tegel') || str_contains($clean, 'teraso')) {
            return 'Ubin / Tegel / Teraso';
        }
        if (str_contains($clean, 'parket') || str_contains($clean, 'vinil') || str_contains($clean, 'karpet')) {
            return 'Parket / Vinil / Karpet';
        }
        if (str_contains($clean, 'kayu') || str_contains($clean, 'papan') || str_contains($clean, 'bambu')) {
            return 'Kayu / Papan';
        }
        if (str_contains($clean, 'tanah')) {
            return 'Tanah';
        }
        if (str_contains($clean, 'semen') || str_contains($clean, 'bata') || str_contains($clean, 'plester') || str_contains($clean, 'cor')) {
    ...
    private function parseWallMaterial(?string $val): ?string
    {
        ...
        if (str_contains($clean, 'tembok') || str_contains($clean, 'bata') || str_contains($clean, 'beton')) {
            return 'Tembok';
        }
        if (str_contains($clean, 'seng') || str_contains($clean, 'zink')) {
            return 'Seng';
        }
        if (
    ...
    private function parseRoofMaterial(?string $val): ?string
    {
        ...
        if (str_contains($clean, 'genteng') || str_contains($clean, 'keramik atap')) {
            return 'Genteng';
        }
        if (str_contains($clean, 'seng') || str_contains($clean, 'zink') || str_contains($clean, 'spandek')) {
            return 'Seng';
        }
        if (str_contains($clean, 'asbes') || str_contains($clean, 'asb')) {
            return 'Asbes';
        }
    ...
    private function parseCondition(?string $val): ?string
    {
        ...
        if (str_contains($clean, 'rusak berat') || str_contains($clean, 'parah')) {
            return 'Rusak Berat';
        }
        if (str_contains($clean, 'rusak sedang') || str_contains($clean, 'sedang')) {
            return 'Rusak Sedang';
        }
        if (str_contains($clean, 'rusak ringan') || str_contains($clean, 'ringan')) {
            return 'Rusak Ringan';
        }
        if (str_contains($clean, 'baik') || str_contains($clean, 'bagus') || str_contains($clean, 'layak')) {
            return 'Baik';
        }
        if (str_contains($clean, 'rusak')) {
            return 'Rusak Ringan';
        }
    ...
    private function parseClosetType(?string $val): ?string
    {
        ...
        if (str_contains($clean, 'leher angsa') || str_contains($clean, 'angsa')) {
            return 'Leher Angsa';
        }
        if (str_contains($clean, 'plengsengan') || str_contains($clean, 'tutup')) {
            return 'Plengsengan dengan Tutup';
        }
    ...
    private function parseFecesDisposal(?string $val): ?string
    {
        ...
        if (str_contains($clean, 'tangki') || str_contains($clean, 'septik') || str_contains($clean, 'septic')) {
            return 'Tangki Septik';
        }
        if (str_contains($clean, 'kolam') || str_contains($clean, 'sawah') || str_contains($clean, 'sungai') || str_contains($clean, 'danau')) {
            return 'Kolam / Sawah / Sungai / Danau';
        }
        if (str_contains($clean, 'lubang') || str_contains($clean, 'tanah')) {
            return 'Lubang Tanah';
        }
    ...
    private function parseWaterSource(?string $val): ?string
    {
        ...
        if (str_contains($clean, 'kemasan') || str_contains($clean, 'galon') || str_contains($clean, 'aqua') || str_contains($clean, 'bermerek')) {
            return 'Air kemasan bermerek';
        }
        if (str_contains($clean, 'leding') || str_contains($clean, 'ledeng') || str_contains($clean, 'pdam') || str_contains($clean, 'pam')) {
            return 'Leding';
        }
        if (str_contains($clean, 'bor') || str_contains($clean, 'pompa') || str_contains($clean, 'artesis')) {
            return 'Sumur Bor / Pompa';
        }
        if (str_contains($clean, 'terlindung') || str_contains($clean, 'terlindungi')) {
            return 'Sumur Terlindung';
        }
        if (str_contains($clean, 'mata air')) {
            return 'Mata Air';
        }
    ...
    private function parseLightingSource(?string $val): ?string
    {
        ...
        if (str_contains($clean, 'non-pln') || str_contains($clean, 'non pln') || str_contains($clean, 'bukan pln') || str_contains($clean, 'genset') || str_contains($clean, 'solar sel')) {
            return 'Listrik Non-PLN';
        }
        if (str_contains($clean, 'tanpa meteran') || str_contains($clean, 'numpang') || str_contains($clean, 'tanpa meter')) {
            return 'Listrik PLN Tanpa Meteran';
        }
        if (str_contains($clean, 'pln') || str_contains($clean, 'meteran') || str_contains($clean, 'meter')) {
            return 'Listrik PLN Dengan Meteran';
        }
        if (str_contains($clean, 'bukan listrik') || str_contains($clean, 'tidak ada listrik') || str_contains($clean, 'lampu minyak') || str_contains($clean, 'lilin')) {
    ...
    private function parseFamilyRelation(?string $val): ?string
    {
        ...
        if (strpos($clean, 'istri') !== false || strpos($clean, 'suami') !== false) {
            return 'Istri';
        }
        if (strpos($clean, 'menantu') !== false) {
            return 'Menantu';
        }
        if (strpos($clean, 'pembantu') !== false || strpos($clean, 'asisten') !== false) {
            return 'Pembantu';
        }
        if (strpos($clean, 'anak') !== false) {
            return 'Anak';
        }
        if (strpos($clean, 'cucu') !== false) {
            return 'Cucu';
        }
        if (strpos($clean, 'mertua') !== false) {
            return 'Mertua';
        }
        if (
        ...
        if (strpos($clean, 'famili') !== false || strpos($clean, 'saudara') !== false || strpos($clean, 'keponakan') !== false) {
            return 'Famili Lain';
        }
