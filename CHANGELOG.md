...
- **Integrasi Widget Aksesibilitas Mandiri Sinjai (`a11y-sinjaikab`)**:
  ...
  - Memuat skrip dari CDN jsdelivr dengan atribut `defer` pada [layouts/app.blade.php](file:///Users/abedzul/Desktop/htdocs/desa-cms/resources/views/layouts/app.blade.php) dengan posisi tombol dinamis (`data-position`), serta membiarkan widget memakai warna bawaan resmi aksesibilitas Sinjai (`#0056b3`) agar konsisten dan tidak terdistorsi warna tema website.
...
- **Uji Coba Otomatisasi**: Menambahkan komentar pada file rute (`routes/web.php`) untuk melakukan uji coba _script_ deployment multi-server.
...
- **Integritas Berkas Migrasi**: Menambahkan bidang `order` dan `is_active` secara langsung pada skema awal tabel `statistic_indicators` (`2026_07_01_151100_create_statistics_tables.php`) dan menambahkan fungsi pemeriksa ketersediaan kolom (`hasColumn`) ke dalam tahapan _rollback_ (metode `down()`), sehingga eksekusi `php artisan migrate:fresh --seed` berjalan 100% lancar tanpa galat *Column not found: 1054 Unknown column 'order'*.
...
### Added
- **Kolom Tipe Domisili Penduduk**: Menambahkan kolom `domicile_address_type` pada tabel `citizens` untuk menyimpan status kesesuaian alamat (Sesuai KK dan KTP, Hanya sesuai KK, Hanya sesuai KTP) dari Kolom 304 via migrasi `2026_07_24_112906_add_domicile_and_rental_columns.php`.
- **Kolom Biaya Sewa Bangunan**: Menambahkan 3 kolom baru pada tabel `families` untuk menyimpan data biaya sewa: `rental_estimate` (perkiraan sewa, Kolom 203.a), `rental_free_estimate` (perkiraan sewa bebas/dinas, Kolom 203.b), dan `rental_contract_value` (nilai kontrak, Kolom 203.c) via migrasi `2026_07_24_112907_add_rental_columns_to_families.php`.
- **3 Kolom Meteran Listrik Terpisah**: Memisah pengisian dan penyimpanan daya listrik keluarga menjadi 3 kolom meteran terpisah: `electricity_power_meter_1`, `electricity_power_meter_2`, dan `electricity_power_meter_3` di tabel `families` dan form Filament via migrasi `2026_07_24_114737_split_electricity_power_into_three_meter_columns_in_families.php`.
- **Statistik Kepemilikan Dompet Digital / Rekening**: Menambahkan kategori dan indikator statistik baru untuk kepemilikan rekening/dompet digital aktif warga (`has_digital_wallet`) di halaman statistik publik via migrasi `2026_07_24_115625_sync_has_digital_wallet_statistic_indicators.php`.

### Changed
- **Standarisasi Format Title Case**: Merapikan dan menyelaraskan penulisan nilai `education_level`, `job`, dan `job_status` pada tabel `citizens` dan `statistic_indicators` menjadi *Title Case* dengan spasi rapi (`SD / Sederajat`, `SMP / Sederajat`, `SMA / Sederajat`, `Buruh / Karyawan / Pegawai Swasta`, dll) via migrasi `2026_07_24_120431_standardize_education_job_and_status_in_citizens_and_indicators.php`.
- **Penggunaan Status Pekerjaan pada Grafik Beranda**: Mengubah grafik demografi pekerjaan di halaman utama (Beranda) agar menggunakan variabel **Status Pekerjaan** (`job_status`) untuk visualisasi yang lebih padat dan informatif.
- **Penyederhanaan Tabel Admin**: Menyembunyikan kolom non-Excel (`place_of_birth`, `religion`, `blood_type`) pada tabel Penduduk, serta menghapus kolom `assistance_type`, `cow_count`, `goat_count`, dan `buffalo_count` dari tabel keluarga secara default.
- **Visualisasi Grafik Batang Statistik**: Menyembunyikan label kategori sumbu X pada grafik batang 1 arah untuk tampilan yang lebih bersih, dan menampilkannya kembali secara otomatis pada grafik 2 arah (*multi-series/split gender*).

### Fixed
- **Bug PIP Tidak Pernah Tersimpan**: Memperbaiki *needle* pencarian kolom PIP pada impor Excel Penduduk (`ListCitizens.php`) dengan menambahkan `'pip'` (pendek) ke dalam array needle agar cocok dengan header `"PIP"` di Excel.
- **Bug Alamat Penduduk Tidak Pernah Diisi dari Data Keluarga**: Memperbaiki pemetaan kolom **304 (Alamat Domisili)** pada impor Excel Penduduk. Kolom enum status disalurkan ke `domicile_address_type`, sementara `address` warga selalu diambil dari data keluarga (`family->address`).
- **Bug Deteksi Kolom 208 (Kondisi Lantai/Dinding/Atap) Ambigu**: Memperbaiki *needle* pencarian kolom 208 pada impor Excel Keluarga (`ListFamilies.php`) menggunakan needle presisi `['[lantai]']`, `['[dinding]']`, dan `['[atap]']`.
- **Pembersihan Duplikasi Data Meteran Listrik**: Menambahkan migrasi pembersihan data `2026_07_24_115115_fix_duplicate_electricity_meter_values_in_families.php` untuk mereset meteran 2 dan 3 yang terisi otomatis dari data gabungan lama.
...
- **Pemetaan Tingkat Pendidikan (Kolom 311)**: Menyempurnakan pemetaan tingkat pendidikan dari **Kolom 311** (`311. Ijazah/STTB tertinggi yang dimiliki`) pada impor Excel Penduduk untuk mendukung seluruh kategori riil (`SD/sederajat`, `SMP/sederajat`, `SMA/sederajat`, `D1/D2/D3`, `D4/S1/Profesi`, `S2/S3`, dan `Tidak punya ijazah SD`), merapikan penulisan nama indikator ke format *Title Case* yang rapi (`Tidak Punya Ijazah SD`, `SD / Sederajat`, `SMP / Sederajat`, `SMA / Sederajat`, `D1 / D2 / D3`, `D4 / S1 / Profesi`, `S2 / S3`), serta menambahkan migrasi perbaikan `2026_07_23_171419_fix_education_indicator_display_names.php`.
- **Pemetaan Profesi Pekerjaan (Kolom 313)**: Menyempurnakan parser `parseJob()` pada pengunggah Excel Penduduk untuk memetakan **Kolom 313** (`313. Profesi Pekerjaan Utama`) ke dalam 12 kategori bidang profesi riil (`Belum / Tidak Bekerja`, `Ibu Rumah Tangga`, `Petani / Pekebun`, `Pelajar / Mahasiswa`, `Wiraswasta / Pengusaha`, `Karyawan Swasta`, `Tenaga Pendidikan`, `Tenaga Kesehatan`, `PNS / Aparatur`, `Buruh / Pekerja Harian`, `Pekerja Jasa & Transportasi`, `Pensiunan`, dan `Lainnya`), serta menyinkronkan indikator grafik publik via migrasi `2026_07_23_170028_sync_pekerjaan_statistic_indicators.php`.
- **Pemetaan Status Pekerjaan (Kolom 314)**: Menyesuaikan parser `parseJobStatus()` pada pengunggah Excel Penduduk untuk memetakan **Kolom 314** menggunakan teks asli kuesioner Excel dengan penyesuaian khusus (`Berusaha sendiri`, `Buruh/karyawan/pegawai swasta`, `Pekerja bebas`, `Pekerja keluarga/tidak dibayar`, `ASN/TNI/Polri/BUMN/BUMD/pejabat negara` [tanpa kades], `Berusaha dibantu buruh`, dan `Lainnya` [ganti dari Tidak tahu]), merapikan penulisan label grafik publik ke format *Title Case* via migrasi `2026_07_23_165850_format_job_status_indicator_names_title_case.php`, serta meregistrasikan `job_status` ke dalam `ALLOWED_COLUMNS` dan generator indikator otomatis di model `StatisticCategory`.
