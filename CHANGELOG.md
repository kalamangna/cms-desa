...
### Added
- **Kolom Domisili Penduduk**: Menambahkan 4 kolom baru pada tabel `citizens` untuk menyimpan data domisili yang sebelumnya tidak tersimpan: `domicile_address_type` (tipe kesesuaian alamat, Kolom 304), `domicile_province` (provinsi domisili, Kolom 305DN.a), `domicile_city` (kabupaten/kota domisili, Kolom 305DN.b), dan `domicile_country` (negara domisili, Kolom 305LN) via migrasi `2026_07_24_112906_add_domicile_and_rental_columns.php`.
- **Kolom Biaya Sewa Bangunan**: Menambahkan 3 kolom baru pada tabel `families` untuk menyimpan data biaya sewa yang sebelumnya tidak tersimpan: `rental_estimate` (perkiraan sewa, Kolom 203.a), `rental_free_estimate` (perkiraan sewa bebas/dinas, Kolom 203.b), dan `rental_contract_value` (nilai kontrak, Kolom 203.c) via migrasi `2026_07_24_112907_add_rental_columns_to_families.php`.

### Fixed
- **Bug PIP Tidak Pernah Tersimpan**: Memperbaiki *needle* pencarian kolom PIP pada impor Excel Penduduk (`ListCitizens.php`) dengan menambahkan `'pip'` (pendek) ke dalam array needle. Header Excel aktual di beberapa dusun hanya menggunakan teks `"PIP"` (satu kata) sehingga needle sebelumnya (`'program indonesia pintar'`, `'bantuan pip'`) tidak pernah cocok — menyebabkan `pip_status = 0` untuk seluruh warga di semua dusun.
- **Bug Alamat Penduduk Tidak Pernah Diisi dari Data Keluarga**: Memperbaiki logika pemetaan kolom **304 (Alamat Domisili)** pada impor Excel Penduduk. Kolom ini berisi nilai *enum* status (`Sesuai KK dan KTP`, `Hanya sesuai KK`, `Hanya sesuai KTP`) yang sebelumnya masuk ke kolom `address` — menyebabkan kondisi `if (empty($address))` selalu `false` dan alamat dari data keluarga tidak pernah di-*copy* ke warga. Nilai enum kini disimpan terpisah ke kolom `domicile_address_type`, sementara `address` selalu diambil dari data keluarga (`family->address`).
- **Bug Deteksi Kolom 208 (Kondisi Lantai/Dinding/Atap) Ambigu**: Memperbaiki *needle* pencarian kolom 208 pada impor Excel Keluarga (`ListFamilies.php`). Sebelumnya, ketiga kolom (lantai, dinding, atap) menggunakan needle yang sama (`'kondisi lantai, dinding'`) sehingga `$colWallCond` dan `$colRoofCond` menunjuk ke kolom yang sama dengan `$colFloorCond`. Needle kini diperbaiki menjadi spesifik: `['[lantai]']`, `['[dinding]']`, dan `['[atap]']`.
- **Penambahan Deteksi Kolom Sewa (203.a/b/c) pada Impor Keluarga**: Menambahkan deteksi dan penyimpanan nilai biaya sewa bangunan (kolom 203.a, 203.b, 203.c) yang sebelumnya tidak dipetakan ke database, ke dalam `rental_estimate`, `rental_free_estimate`, dan `rental_contract_value`.
