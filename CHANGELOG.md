...
- **Pemetaan Status Pekerjaan (Kolom 314)**: Menyesuaikan parser `parseJobStatus()` pada pengunggah Excel Penduduk untuk memetakan **Kolom 314** menggunakan teks asli kuesioner Excel dengan penyesuaian khusus (`Berusaha sendiri`, `Buruh/karyawan/pegawai swasta`, `Pekerja bebas`, `Pekerja keluarga/tidak dibayar`, `ASN/TNI/Polri/BUMN/BUMD/pejabat negara` [tanpa kades], `Berusaha dibantu buruh`, dan `Lainnya` [ganti dari Tidak tahu]), serta merapikan penulisan label grafik publik ke format *Title Case* via migrasi `2026_07_23_165850_format_job_status_indicator_names_title_case.php`.
