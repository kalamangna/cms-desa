...
@section('title', 'Statistik Desa | Desa ' . ($site_settings['village_name'] ?? ''))
    ...
    <div class="flex gap-8 items-start">
        ...
        <div class="flex-1 min-w-0">
            ...
            @php
                ...
                $allYears = $category->indicators->flatMap(fn($i) => $i->data ? $i->data->pluck('year') : collect())->unique()->sort()->values();
            ...
            <div x-show="activeTab === '{{ $category->slug }}'"
                 ...
                 x-data="{
                     ...
                     filterYear: 'all',
                     compareYear: 'none',
                     activeSecondaryKey: 'none',
                     renderChart() {
                         window.renderStatChart('{{ $category->slug }}', this.currentType, this.showPercent, this.filterYear, this.compareYear, this.activeSecondaryKey);
                     }
                 }"
                ...
                <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-700 shadow-lg shadow-slate-200/50 dark:shadow-slate-950/50 overflow-hidden p-6 md:p-8 mb-6">
                    ...
                    <div class="relative min-h-[380px]">
                        <div id="chart-{{ $category->slug }}" class="w-full"></div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-700 shadow-lg shadow-slate-200/50 dark:shadow-slate-950/50 overflow-hidden mb-8">
                    ...
                    <div class="overflow-x-auto">
                        @php
                            $firstIndicatorUnit = $category->indicators->first()->unit ?? ($isCitizens ? 'Jiwa' : 'Keluarga');
                        @endphp
                            ...
                            <tbody id="tbody-{{ $category->slug }}" class="divide-y divide-slate-50 dark:divide-slate-800/50">
                                @php
                                    ...
                                    foreach ($category->indicators as $ind) {
                                        $dp = $ind->data ? $ind->data->firstWhere('year', $tableYear) : null;
                                        $grandTotal += $dp ? (int)($dp->value ?? 0) : 0;
                                    }
                                @endphp
                                @foreach($category->indicators as $indicator)
                                @php
                                    $dp  = $indicator->data ? $indicator->data->firstWhere('year', $tableYear) : null;
...
<script>
    ...
    const categoryData = {
        ...
        '{{ $category->slug }}': {
            ...
            secondaryConfigs: {!! json_encode($category->secondaryConfigs ?? new stdClass) !!},
            ...
            indicators: [
                ...
                @endphp
                {
                    ...
                    color: '{{ $c }}',
                    breakdowns: {!! json_encode($indicator->breakdowns ?? new stdClass) !!},
    ...
    // ── Update Dynamic Table DOM ─────────────────────────────────────────────
    window.updateStatTableDOM = function(slug, secondaryKey, selectedYear) {
        const cat = categoryData[slug];
        if (!cat) return;
        const table = document.getElementById('tabel-' + slug);
        const thead = document.getElementById('thead-' + slug);
        ...
        const tableYear = selectedYear ? Number(selectedYear) : (cat.years.length ? Math.max(...cat.years.map(Number)) : new Date().getFullYear());
        const firstUnit = sortedIndicators[0]?.unit || (cat.isCitizens ? 'Jiwa' : 'Keluarga');

        const secConf = (secondaryKey && secondaryKey !== 'none' && cat.secondaryConfigs) ? cat.secondaryConfigs[secondaryKey] : null;

        if (secConf && secConf.options && secConf.options.length > 0) {
            const options = secConf.options;
            const colors = secConf.colors || [];
            ...
            const colSums = {};
            options.forEach(opt => colSums[opt] = 0);
            let tbodyHtml = '';

            sortedIndicators.forEach(ind => {
                const d = ind.data.find(d => Number(d.year) === tableYear);
                const indTotal = d ? (d.value || 0) : 0;
                ...
                options.forEach((opt, idx) => {
                    const val = indBreakdown[opt] ?? 0;
                    colSums[opt] += val;
                    const colColor = colors[idx] || '#64748b';
                    const pctCell = indTotal > 0 ? (Math.round((val / indTotal) * 1000) / 10) : 0;

                    tbodyHtml += `<td class="px-4 py-3.5 text-right whitespace-nowrap">
                        <span class="text-xs font-bold" style="color: ${colColor}">${val.toLocaleString('id-ID')}</span>
                        ${indTotal > 0 ? `<span class="text-[10px] font-semibold ml-1 opacity-80" style="color: ${colColor}">(${pctCell.toString().replace('.', ',')}%)</span>` : ''}
                    </td>`;
                });
            ...
            if (tfoot) {
                ...
                options.forEach((opt, idx) => {
                    ...
                    const colColor = colors[idx] || '#64748b';
                    const pctCol = grandTotal > 0 ? (Math.round((sumVal / grandTotal) * 1000) / 10) : 0;

                    tfootHtml += `<td class="px-4 py-3.5 text-right text-xs font-black whitespace-nowrap" style="color: ${colColor}">
    ...
    window.renderStatChart = function(slug, type, showPercent, filterYear, compareYear, secondaryKey) {
        ...
        // Render dynamic table matching chart
        window.updateStatTableDOM(slug, secondaryKey, filterYear !== 'all' ? filterYear : null);

        // ── DYNAMIC SECONDARY BREAKDOWN CHART (HORIZONTAL STACKED BAR) ────
        if (compareYear === 'none' && secondaryKey && secondaryKey !== 'none' && cat.secondaryConfigs && cat.secondaryConfigs[secondaryKey]) {
            const secConf = cat.secondaryConfigs[secondaryKey];
            const options = secConf.options || [];
            const secColors = secConf.colors || [];
            ...
            const series = options.map((opt) => {
                return {
                    ...
                    data: filteredIndicators.map(ind => {
                        const raw = ind.breakdowns?.[secondaryKey]?.[opt] ?? 0;
                        const indData = ind.data.find(d => Number(d.year) === yr);
                        const indTotal = indData ? indData.value : 0;
                        if (showPercent && indTotal > 0) {
                            return Math.round((raw / indTotal) * 1000) / 10;
                        }
                        return raw;
                    })
                };
            ...
            const calculatedHeight = Math.max(380, filteredIndicators.length * 48);

            const config = {
                chart: {
                    type: 'bar',
                    stacked: true,
                    stackType: showPercent ? '100%' : undefined,
        ...
        const getValue = (ind, yr) => {
            ...
            if (showPercent && grandTotals[yr] > 0) {
                return Math.round((raw / grandTotals[yr]) * 1000) / 10;
            }
            return raw;
        };
        ...
        if (type === 'bar' || type === 'line') {
            ...
            if (compareYearNum !== null && activeYears.length === 1) {
            ...
            const colors = filteredIndicators.map((i, idx) => i.color || palette[idx % palette.length]);

            const xCats = (compareYearNum !== null && activeYears.length === 1)
                ...
                try {
                    ...
                    Object.keys(json).forEach(slug => {
                        if (categoryData[slug]) {
                            ...
                            if (newCat.secondaryConfigs) {
                                categoryData[slug].secondaryConfigs = newCat.secondaryConfigs;
                            }
                        }
                    });
            ...
            renderActiveChart() {
                ...
                setTimeout(() => {
                    ...
                    if (chartCard && chartCard._x_dataStack && chartCard._x_dataStack[0]) {
                        ...
                        window.renderStatChart(this.activeTab, state.currentType || 'bar', state.showPercent || false, state.filterYear || 'all', state.compareYear || 'none', state.activeSecondaryKey || 'none');
                    } else {
                        window.renderStatChart(this.activeTab, 'bar', false, 'all', 'none', 'none');
                    }
                }, 50);
            },
            updateUrl(slug) {
