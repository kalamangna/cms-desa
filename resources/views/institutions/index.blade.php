...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-20 lg:py-28"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        ...
        <div class="group flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left bg-white rounded-3xl p-6 md:p-8 border border-slate-100 shadow-sm hover:shadow-lg hover:border-emerald-200 hover:-translate-y-1 overflow-hidden transition-all duration-300 gap-6 h-full">

            {{-- Logo --}}
            <div class="flex-shrink-0">
                <div class="w-28 h-28 md:w-32 md:h-32 rounded-full overflow-hidden ring-4 ring-white shadow-xl shadow-slate-300/40 group-hover:ring-emerald-200 transition-all duration-300 mx-auto sm:mx-0">
                    <img
            ...
            <div class="flex-1 w-full flex flex-col justify-between h-full">
                <div class="mb-5">
                    <h3 class="text-xl md:text-2xl font-heading font-bold text-slate-900 mb-3 group-hover:text-emerald-600 transition-colors">{{ $institution->name }}</h3>
                ...
                <button type="button"
                        ...
                        class="w-full sm:w-auto mt-auto inline-flex items-center justify-center gap-2 bg-slate-50 hover:bg-emerald-50 text-slate-600 hover:text-emerald-700 py-3 px-5 rounded-2xl text-xs font-bold border border-slate-100 hover:border-emerald-100 transition-all duration-300 cursor-pointer">
    ...
    <div x-show="modalOpen"
         ...
         <div class="relative w-[92vw] sm:w-[85vw] md:w-[75vw] lg:w-[60vw] max-w-4xl bg-white rounded-2xl md:rounded-[28px] overflow-hidden shadow-2xl border border-slate-100 flex flex-col max-h-[90vh] cursor-default"
              ...
              <div class="p-6 sm:p-8 bg-slate-50 border-b border-slate-100 flex flex-col sm:flex-row items-center gap-6 sm:gap-8 flex-shrink-0">
              ...
              <div class="p-6 sm:p-8 bg-white overflow-y-auto custom-scrollbar flex-1">
                  ...
                  <template x-if="selectedInstitution?.management && selectedInstitution.management.length > 0">
                      <div class="mt-10 border-t border-slate-100 pt-8">
                          <h4 class="text-sm font-black uppercase tracking-widest text-emerald-600 mb-6">
                              Struktur Pengurus
                          </h4>
                          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                              <template x-for="member in selectedInstitution.management" :key="member.name + member.position">
                                  <div class="flex flex-col p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:border-emerald-200 hover:bg-emerald-50/30 transition-colors">
                                      <span class="text-[10px] font-black uppercase tracking-wider text-emerald-600/80 mb-1" x-text="member.position"></span>
